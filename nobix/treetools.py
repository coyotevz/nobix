...
# Current stable version 0.9.9.1 of urwid dont have this feature (09/Jul/2010)
# Future module from urwid mercurial revision 428:3890a8755c5b
...
#
# Added by Augusto Roccasalva
#

class TreeListWalker(urwid.ListWalker):

    def __init__(self, root_nodes):
        if not type(root_nodes) == list and not hasattr(root_nodes, '__getitem__'):
            raise urwid.ListWalkerError("TreeListWalker expecting list like object, got: %r" % (root_nodes,))
        self._nodes = root_nodes
        self.focus = None

    def get_focus(self):
        """Return (focus widget, focus node)"""
        if len(self._nodes) == 0:
            return None, None
        if self.focus is None:
            return self._nodes[0].get_widget(), self._nodes[0]
        return self.focus.get_widget(), self.focus
    ...
    def get_next(self, start_node):
        """Return (widget after start_node, node after start_node)."""
        # 1. try for inner nodes
        widget = start_node.get_widget()
        target = widget.next_inorder()
        if target is not None:
            return target, target.get_node()

        # 2. try for root nodes
        root = start_node.get_root()

        pos = self._nodes.index(root) + 1
        if len(self._nodes) <= pos:
            return None, None
        widget = self._nodes[pos].get_widget()
        return widget, widget.get_node()

    def get_prev(self, start_node):
        """Return (widget before start_node, node before start_node)."""
        # 1. try for inner nodes
        widget = start_node.get_widget()
        target = widget.prev_inorder()
        if target is not None:
        ...
        # 3. try for last child expanded of previous node
        lastchild = widget.last_child()
        if lastchild is not None:
            return lastchild, lastchild.get_node()

        # 4. finally return previous root node
        return widget, widget.get_node()

#
# End added by Augusto Roccasalva
#
...
class TreeListBox(urwid.ListBox):
    ...
    def focus_home(self, size):
        ...
        if rootwidget.is_leaf or not rootwidget.expanded:
            return
    ...
    def focus_end( self, size ):
        ...
        # here lastwidget can be None if the node is collapsed
...
# Extra generic nodes, widgets
class DictChildWidget(TreeWidget):
    def get_display_text(self):
        return unicode(self.get_node().get_value())

class DictWidget(TreeWidget):
    def get_display_text(self):
        return unicode(self.get_node().get_value().get('display_text', ''))

class DictChildNode(TreeNode):
    def load_widget(self):
        return DictChildWidget(self)

class DictNode(ParentNode):
    """Generic node expecting a dict with 'display_text' and 'childs' keys"""
    def load_widget(self):
        return DictWidget(self)

    def load_child_keys(self):
        qchilds = len(self.get_value().get('childs', []))
        return range(qchilds)

    def load_child_node(self, key):
        child = self.get_value()['childs'][key]
        childdepth = self.get_depth() + 1
        if isinstance(child, dict):
            return DictNode(child, parent=self, key=key, depth=childdepth)
        return DictChildNode(child, parent=self, key=key, depth=childdepth)
