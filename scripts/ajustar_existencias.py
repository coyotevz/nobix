...
def ajustar():
    ...
    bar = ProgressBar(u"ajustando existencias", query.count(), suffix=False)
    counter = count(1)

    for articulo in query:
        qty = zero
        for item in session.query(ItemDocumento).filter(ItemDocumento.articulo_id==articulo.id)\
                    .outerjoin(ItemDocumento.documento).options(contains_eager(ItemDocumento.documento))\
                    .order_by(Documento.fecha):
            doctype = config.documentos[item.documento.tipo]
            mov = doctype['stock']
            if mov == u'inventario':
                qty = item.cantidad
            elif mov in ('entrada', 'ajuste'):
                qty += item.cantidad
            elif mov == 'salida':
                qty -= item.cantidad
        articulo.existencia = qty
        bar.update_state(counter.next())
    session.commit()
    bar.finish()
